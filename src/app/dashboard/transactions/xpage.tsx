...
export default function TransactionsPage() {
  ...
  async function submitEscrowRequest() {
    ...
    await updateTx(escrowTx.id, { escrow_status: 'requested', escrow_amount: amt, escrow_ref: escrowRef || null })
  ...
  function ActionButtons({ tx }: { tx: Tx }) {
    ...
    if (s === 'offer_sent') {
      ...
      if (isBuyer)  actions.push(btn('✎ Counter Offer', '#1d4ed8', '#eff6ff', () => setExpanded(tx.id)))
      actions.push(btn('✕ Cancel', '#dc2626', '#fef2f2', () => updateTx(tx.id, { status: 'cancelled' })))
    }

    // Confirmed stage
    if (s === 'confirmed') {
      if (isBuyer && tx.escrow_status === 'none') actions.push(btn('🔒 Request Escrow', '#6d28d9', '#f5f3ff', () => { setEscrowTx(tx); setEscrowAmount(tx.price && tx.quantity ? String(tx.price * tx.quantity) : '') }))
      ...
      {escrowTx && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 24, maxWidth: 400, width: '100%' }}>
            ...
            <div style={{ fontSize: 13, color: '#64748b', marginBottom: 14, lineHeight: 1.6 }}>Payment will be held in your escrow account until you confirm goods received. Requires sufficient escrow balance.</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div>
                <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 3, fontWeight: 600, textTransform: 'uppercase' as const }}>Escrow Amount (EUR)</label>
                <input type="number" value={escrowAmount} onChange={e => setEscrowAmount(e.target.value)} placeholder="Amount to hold..." style={{ width: '100%', padding: '8px 11px', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
              <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={submitEscrowRequest} style={{ flex: 1, padding: '10px', background: '#6d28d9', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: 700 }}>Request Escrow</button>
