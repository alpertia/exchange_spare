...
export default function SellingPage() {
  ...
  async function loadGroups(cid: string) {
    ...
    // "All Listings" always first
    const total = gs.reduce((s, g) => s + g.count, 0)
    const allGroup: ImportGroup = { id: '__all__', name: '📋 All Listings', created_at: '', count: total, source: 'csv' }
    const finalGs = total > 0 ? [allGroup, ...gs] : gs
    setGroups(finalGs)
  ...
  const loadListings = useCallback(async (cid: string, groupId: string) => {
    ...
    if (groupId === '__all__') { /* no filter — load all */ }
    else if (groupId === 'manual') q.is('import_batch', null)
  ...
  async function importCSV() {
    ...
    let ok = 0, fail = 0
    const errors: { pn: string; reason: string }[] = []

    // ── Diff: load existing listings for this batch to compare ──
    const { data: existingListings } = await supabase.from('listings')
      .select('id, product:product_id(normalized_pn), quantity, price, currency, condition')
      .eq('company_id', myId)
      .eq('import_batch', batchName)
      .eq('status', 'active')
    const existingPNs = new Map<string, any>()
    ;(existingListings || []).forEach((l: any) => {
      if (l.product?.normalized_pn) existingPNs.set(l.product.normalized_pn, l)
    })
    const newPNs = new Set(csvRows.map(r => r.pn))

    // Track diff
    const added: string[] = []
    const updated: { pn: string; changes: string }[] = []

    for (const row of csvRows) {
      try {
        // Validate required fields
        if (!row.pn || row.pn.length < 2) { errors.push({ pn: row.pn || '(empty)', reason: 'Invalid or empty part number' }); fail++; continue }

        let { data: prod } = await supabase.from('products').select('id').eq('normalized_pn', row.pn).maybeSingle()
        if (!prod) {
          const { data: np, error: prodErr } = await supabase.from('products').insert({
            normalized_pn: row.pn,
            brand: row.brand || 'UNKNOWN',
            description: row.description || null,
          }).select().single()
          if (prodErr || !np) { errors.push({ pn: row.pn, reason: `Product create failed: ${prodErr?.message || 'unknown'}` }); fail++; continue }
          prod = np
        }

        // Check duplicate within SAME batch
        const { data: existInBatch } = await supabase.from('listings')
          .select('id')
          .eq('company_id', myId)
        ...
        if (existInBatch) {
          ...
          }).eq('id', existInBatch.id)
          if (updErr) { errors.push({ pn: row.pn, reason: `Update failed: ${updErr.message}` }); fail++; continue }

          // Track what changed for diff
          const old = existingPNs.get(row.pn)
          if (old) {
            const changes: string[] = []
            if (old.quantity !== (parseInt(row.qty) || null)) changes.push(`qty: ${old.quantity}→${row.qty || '—'}`)
            if (old.price !== (parseFloat(row.price) || null)) changes.push(`price: ${old.price}→${row.price || '—'}`)
            if (changes.length > 0) updated.push({ pn: row.pn, changes: changes.join(', ') })
          }
          ok++
        } else {
          // Check cross-batch duplicate
          const { count: otherBatchCount } = await supabase.from('listings')
            .select('*', { count: 'exact', head: true })
            .eq('company_id', myId)
            .eq('product_id', prod.id)
            .eq('status', 'active')
            .neq('import_batch', batchName)
          ...
          const { error: insErr } = await supabase.from('listings').insert({
            ...
            cross_list_duplicate: (otherBatchCount || 0) > 0,
          })
          if (insErr) { errors.push({ pn: row.pn, reason: `Insert failed: ${insErr.message}` }); fail++; continue }

          // Is this a new PN not in previous batch?
          if (!existingPNs.has(row.pn)) added.push(row.pn)
          ok++
        }
      } catch (e: any) {
        errors.push({ pn: row.pn, reason: e?.message || 'Unexpected error' })
        fail++
      }
    }

    // Removed = was in old batch, not in new file
    const removed = [...existingPNs.keys()].filter(pn => !newPNs.has(pn))
    const unchanged = ok - added.length - updated.length

    // Build diff report only if re-importing existing batch
    if (existingPNs.size > 0) {
      setDiffReport({ added, removed, updated, unchanged: Math.max(0, unchanged) })
    }

    setImportDone({ ok, fail, errors })
    setImporting(false)
        ...
        <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 10, padding: 24, maxWidth: 800 }}>
          ...
          {importDone && (
            <div style={{ padding: '20px 0' }}>
              ...
              {importDone.errors.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  <button onClick={() => setShowErrors(!showErrors)}
                    style={{ padding: '8px 14px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#dc2626', width: '100%', textAlign: 'left' }}>
                    ...
                    <div style={{ maxHeight: 200, overflowY: 'auto', border: '1px solid #fecaca', borderTop: 'none', borderRadius: '0 0 6px 6px', background: '#fff5f5' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                        <thead><tr style={{ background: '#fef2f2' }}>
                          <th style={{ padding: '6px 10px', textAlign: 'left', color: '#dc2626', fontWeight: 700 }}>PN</th>
                          <th style={{ padding: '6px 10px', textAlign: 'left', color: '#dc2626', fontWeight: 700 }}>Error</th>
                        </tr></thead>
                        <tbody>
                          {importDone.errors.map((e, i) => (
                            <tr key={i} style={{ borderTop: '1px solid #fecaca' }}>
                              <td style={{ padding: '5px 10px', fontFamily: 'monospace', fontWeight: 700, color: '#0f172a' }}>{e.pn}</td>
              ...
              {/* Diff report (re-import comparison) */}
              {diffReport && (
                <div style={{ background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 8, padding: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 10 }}>
                    <div style={{ textAlign: 'center', padding: '8px', background: '#ecfdf5', borderRadius: 6 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: '#15803d' }}>{diffReport.added.length}</div>
                      <div style={{ fontSize: 11, color: '#15803d' }}>New PNs</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '8px', background: '#fef2f2', borderRadius: 6 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: '#dc2626' }}>{diffReport.removed.length}</div>
                      <div style={{ fontSize: 11, color: '#dc2626' }}>Removed</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '8px', background: '#eff6ff', borderRadius: 6 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: '#2563eb' }}>{diffReport.updated.length}</div>
                      <div style={{ fontSize: 11, color: '#2563eb' }}>Updated</div>
                    </div>
                    <div style={{ textAlign: 'center', padding: '8px', background: '#f8fafc', borderRadius: 6 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: '#64748b' }}>{diffReport.unchanged}</div>
                      <div style={{ fontSize: 11, color: '#64748b' }}>Unchanged</div>
                    </div>
                  </div>
                  {diffReport.added.length > 0 && (
                    <div style={{ marginBottom: 8 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#15803d', marginBottom: 4 }}>+ New PNs added:</div>
                      <div style={{ fontSize: 11, color: '#15803d', fontFamily: 'monospace', maxHeight: 60, overflowY: 'auto' }}>{diffReport.added.join(', ')}</div>
                    </div>
                  )}
                  {diffReport.removed.length > 0 && (
                    <div style={{ marginBottom: 8 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#dc2626', marginBottom: 4 }}>− Removed from list (no longer in file):</div>
                      <div style={{ fontSize: 11, color: '#dc2626', fontFamily: 'monospace', maxHeight: 60, overflowY: 'auto' }}>{diffReport.removed.join(', ')}</div>
                    </div>
                  )}
                  {diffReport.updated.length > 0 && (
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#2563eb', marginBottom: 4 }}>↻ Updated:</div>
                      <div style={{ maxHeight: 80, overflowY: 'auto' }}>
                        {diffReport.updated.map((u, i) => (
                          <div key={i} style={{ fontSize: 11, color: '#2563eb', fontFamily: 'monospace' }}>{u.pn}: {u.changes}</div>
                        ))}
                      </div>
                    </div>
                  )}
