...
type Row = {
  ...
  manufacture_date: string; stock_entry_date: string
}
type ImportGroup = {
...
export default function SellingPage() {
  ...
  function openEdit(l: Listing) {
    ...
    setEditImgUrl(l.condition_image_url)
    setEditing(true)
  }
  ...
  async function saveSingle() {
    ...
    const { error } = await supabase.from('listings').insert({ company_id: myId, product_id: !prod.id, status: 'active', quantity: parseInt(form.qty) || null, price: parseFloat(form.price) || null, currency: form.currency, condition: form.condition || null, warehouse_location: form.warehouse_location || null, notes: form.notes || null, manufacture_date: form.manufacture_date || null, stock_entry_date: form.stock_entry_date || null, import_batch: null })
                ...
                : allListings.length === 0
                  ...
                  : listings.map(l => (
                    <div key={l.id}
                      ...
                      {/* Thumbnail */}
                      {(l.condition_image_url || l.product?.images?.[0]) ? (
                        <img src={l.condition_image_url || l.product.images![0]} alt=""
              ...
              editing ? (
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
                  <div style={{ marginBottom: 14, display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                    {/* Main image */}
                    {(selectedListing.condition_image_url || selectedListing.product?.images?.[0]) ? (
                      <img src={selectedListing.condition_image_url || selectedListing.product.images![0]} alt=""
                        onClick={() => window.open((selectedListing.condition_image_url || selectedListing.product.images![0])!, '_blank')}
                  ...
                  {selectedListing.condition_image_url && (
                    <div style={{ marginBottom: 14 }}>
                      <div style={{ fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', fontWeight: 700, marginBottom: 6 }}>Condition Photo</div>
                      <img src={selectedListing.condition_image_url} alt="condition" onClick={() => window.open(selectedListing.condition_image_url!, '_blank')}
