...
type Listing = {
  ...
  condition: string | null; warehouse_location: string | null; notes: string | null
  product_image: string | null  // products.images[0]
}
type BuyIntent = {
  id: string; product_id: string; pn: string; brand: string
  company_id: string; dealer_code: string; is_mine: boolean
...
// ── Product thumbnail ──────────────────────────────────────────────────────────
function ProductThumb({ src, size = 48 }: { src: string | null; size?: number }) {
  const [err, setErr] = useState(false)
  if (!src || err) return (
    <div style={{ width: size, height: size, borderRadius: 6, border: '1px solid #e2e8f0', background: '#f8fafc', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: size * 0.45 }}>
      📦
    </div>
  )
  return (
    <img src={src} alt="" onError={() => setErr(true)}
      style={{ width: size, height: size, objectFit: 'cover', borderRadius: 6, border: '1px solid #e2e8f0', flexShrink: 0 }} />
  )
}
...
export default function MarketplacePage() {
  ...
  async function load() {
    ...
    const [{ data: ls }, { data: bi }] = await Promise.all([
      supabase.from('listings')
        .select('id, product_id, quantity, price, currency, condition, warehouse_location, notes, company_id')
        .eq('status', 'active').order('created_at', { ascending: false }),
      supabase.from('trade_intent')
        .select('id, product_id, quantity, target_price, company_id')
        .eq('status', 'active').eq('intent_type', 'buy').order('created_at', { ascending: false }),
    ])

    // Fetch products in chunks to avoid URL length limit
    const allProductIds = [...new Set([
      ...(ls || []).map((r: any) => r.product_id),
      ...(bi || []).map((r: any) => r.product_id),
    ].filter(Boolean))]
    ...
    if (allProductIds.length > 0) {
      ...
      for (let i = 0; i < allProductIds.length; i += CHUNK) {
        ...
        const { data: prods } = await supabase.from('products')
          .select('id, normalized_pn, brand, images')
          .in('id', chunk)
    ...
    setListings((ls || []).map((r: any) => {
      const p = productMap[r.product_id] || {}
      return {
        id: r.id, product_id: r.product_id,
        pn: p.normalized_pn || '', brand: p.brand || '',
        company_id: r.company_id, dealer_code: dealerCode(r.company_id),
        is_mine: r.company_id === mid,
        quantity: r.quantity, price: r.price, currency: r.currency || 'EUR',
        condition: r.condition, warehouse_location: r.warehouse_location, notes: r.notes,
        product_image: p.images?.[0] || null,
      }
    }))
    setIntents((bi || []).map((r: any) => {
      const p = productMap[r.product_id] || {}
      return {
        id: r.id, product_id: r.product_id,
        pn: p.normalized_pn || '', brand: p.brand || '',
        company_id: r.company_id, dealer_code: dealerCode(r.company_id),
        is_mine: r.company_id === mid,
        quantity: r.quantity, target_price: r.target_price,
        product_image: p.images?.[0] || null,
      }
    }))
    setLoading(false)
  ...
  async function offerProduct(intent: BuyIntent) {
    if (!myId.current) { router.push('/login'); return }
    const base = { company_id: myId.current, counterpart_id: intent.company_id, product_id: intent.product_id, type: 'sell', status: 'offer_sent', quantity: intent.quantity, price: intent.target_price, currency: 'EUR', escrow_status: 'none' }
    await supabase.from('transactions').insert(base)
    router.push('/dashboard/transactions')
  }

  const q = query.trim()

  // PNs I have listed (for "You have it!" badge on buy requests)
  const myListingPNs = new Set(listings.filter(l => l.is_mine).map(l => norm(l.pn)))
  // PNs I want to buy (for "We found your request!" on listings)
  const myIntentPNs  = new Set(intents.filter(i => i.is_mine).map(i => norm(i.pn)))

  // Sort listings: my matches first, then by listing count (popular), then rest
  const pnCount: Record<string, number> = {}
  listings.forEach(l => { pnCount[norm(l.pn)] = (pnCount[norm(l.pn)] || 0) + 1 })

  const sortListings = (arr: Listing[]) => [...arr].sort((a, b) => {
    const aMatch = myIntentPNs.has(norm(a.pn)) ? 1 : 0
    const bMatch = myIntentPNs.has(norm(b.pn)) ? 1 : 0
    if (bMatch !== aMatch) return bMatch - aMatch
    return (pnCount[norm(b.pn)] || 0) - (pnCount[norm(a.pn)] || 0)
  })

  const sortIntents = (arr: BuyIntent[]) => [...arr].sort((a, b) => {
    const aMatch = myListingPNs.has(norm(a.pn)) ? 1 : 0
    const bMatch = myListingPNs.has(norm(b.pn)) ? 1 : 0
    return bMatch - aMatch
  })

  const baseListings = q ? listings.filter(l => norm(l.pn).includes(norm(q)) || norm(l.brand).includes(norm(q)) || l.dealer_code.includes(q)) : listings
  const baseIntents  = q ? intents.filter(i => norm(i.pn).includes(norm(q)) || norm(i.brand).includes(norm(q)) || i.dealer_code.includes(q)) : intents
  const fls = sortListings(baseListings)
  const fbi = sortIntents(baseIntents)
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {q && <div style={{ marginTop: 6, fontSize: 12, color: '#94a3b8' }}>{fls.length} available · {fbi.length} wanted for "{q}"  <button style={{ color: '#64748b', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', fontSize: 11 }} onClick={() => { setQuery(''); setListPage(1); setIntentPage(1) }}>clear</button></div>}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {(tab === 'all' || tab === 'selling') && (
            <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
              ...
              {fls.slice((listPage-1)*PAGE_SIZE, listPage*PAGE_SIZE).map(l => (
                <div key={l.id} style={{ padding: '12px 16px', borderBottom: '1px solid #f8fafc', display: 'flex', alignItems: 'center', gap: 12, background: myIntentPNs.has(norm(l.pn)) && !l.is_mine ? '#f0fdf4' : undefined }}>
                  {/* Thumbnail */}
                  <ProductThumb src={l.product_image} size={48} />
                  {/* Info */}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                      <span 
                        onClick={e => { e.stopPropagation(); if (l.product_id) router.push(`/dashboard/knowledge/${l.product_id}`) }}
                        style={{ fontWeight: 700, color: '#0f172a', fontSize: 14, cursor: l.product_id ? 'pointer' : 'default', textDecoration: l.product_id ? 'underline' : 'none', textDecorationStyle: 'dotted', textUnderlineOffset: 3 }}>
                        {l.pn}
                      </span>
                      ...
                      {myIntentPNs.has(norm(l.pn)) && !l.is_mine && (
                        <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: '#dcfce7', color: '#15803d', fontWeight: 700 }}>✓ We found your request!</span>
                      )}
                    </div>
                    <div style={{ fontSize: 12, color: '#64748b', display: 'flex', flexWrap: 'wrap', gap: '0 10px' }}>
                      ...
                      {l.warehouse_location && <span>📍{l.warehouse_location}</span>}
                    </div>
              ...
              {/* Pagination */}
              {fls.length > PAGE_SIZE && (
                <div style={{ padding: '10px 16px', borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{Math.min((listPage-1)*PAGE_SIZE+1, fls.length)}–{Math.min(listPage*PAGE_SIZE, fls.length)} of {fls.length}</span>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => setListPage(p => Math.max(1, p-1))} disabled={listPage === 1}
                      style={{ padding: '4px 10px', background: listPage === 1 ? '#f1f5f9' : 'white', border: '1px solid #e2e8f0', borderRadius: 4, cursor: listPage === 1 ? 'default' : 'pointer', fontSize: 12, color: listPage === 1 ? '#94a3b8' : '#0f172a' }}>← Prev</button>
                    <button onClick={() => setListPage(p => Math.min(Math.ceil(fls.length/PAGE_SIZE), p+1))} disabled={listPage >= Math.ceil(fls.length/PAGE_SIZE)}
                      style={{ padding: '4px 10px', background: listPage >= Math.ceil(fls.length/PAGE_SIZE) ? '#f1f5f9' : 'white', border: '1px solid #e2e8f0', borderRadius: 4, cursor: listPage >= Math.ceil(fls.length/PAGE_SIZE) ? 'default' : 'pointer', fontSize: 12, color: listPage >= Math.ceil(fls.length/PAGE_SIZE) ? '#94a3b8' : '#0f172a' }}>Next →</button>
                  </div>
                </div>
              )}
              </>)}
          ...
          {(tab === 'all' || tab === 'buying') && (
            <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
              ...
              {fbi.slice((intentPage-1)*PAGE_SIZE, intentPage*PAGE_SIZE).map(i => (
                <div key={i.id} style={{ padding: '12px 16px', borderBottom: '1px solid #f8fafc', display: 'flex', alignItems: 'center', gap: 12, background: myListingPNs.has(norm(i.pn)) && !i.is_mine ? '#fffbeb' : '#fffef5' }}>
                  {/* Thumbnail */}
                  <ProductThumb src={i.product_image} size={48} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, flexWrap: 'wrap' }}>
                    <span
                      onClick={e => { e.stopPropagation(); if (i.product_id) router.push(`/dashboard/knowledge/${i.product_id}`) }}
                      style={{ fontWeight: 700, color: '#0f172a', fontSize: 14, cursor: i.product_id ? 'pointer' : 'default', textDecoration: i.product_id ? 'underline' : 'none', textDecorationStyle: 'dotted', textUnderlineOffset: 3 }}>
                      {i.pn}
                    </span>
                    {myListingPNs.has(norm(i.pn)) && !i.is_mine && (
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: '#fef3c7', color: '#92400e', fontWeight: 700 }}>📦 You have it!</span>
                    )}
                  </div>
                    <div style={{ fontSize: 12, color: '#64748b', display: 'flex', flexWrap: 'wrap', gap: '0 10px' }}>
                      <span>{i.brand}</span>
              ...
              {/* Pagination */}
              {fbi.length > PAGE_SIZE && (
                <div style={{ padding: '10px 16px', borderTop: '1px solid #fef3c7', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{Math.min((intentPage-1)*PAGE_SIZE+1, fbi.length)}–{Math.min(intentPage*PAGE_SIZE, fbi.length)} of {fbi.length}</span>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => setIntentPage(p => Math.max(1, p-1))} disabled={intentPage === 1}
                      style={{ padding: '4px 10px', background: intentPage === 1 ? '#f1f5f9' : 'white', border: '1px solid #e2e8f0', borderRadius: 4, cursor: intentPage === 1 ? 'default' : 'pointer', fontSize: 12, color: intentPage === 1 ? '#94a3b8' : '#0f172a' }}>← Prev</button>
                    <button onClick={() => setIntentPage(p => Math.min(Math.ceil(fbi.length/PAGE_SIZE), p+1))} disabled={intentPage >= Math.ceil(fbi.length/PAGE_SIZE)}
                      style={{ padding: '4px 10px', background: intentPage >= Math.ceil(fbi.length/PAGE_SIZE) ? '#f1f5f9' : 'white', border: '1px solid #e2e8f0', borderRadius: 4, cursor: intentPage >= Math.ceil(fbi.length/PAGE_SIZE) ? 'default' : 'pointer', fontSize: 12, color: intentPage >= Math.ceil(fbi.length/PAGE_SIZE) ? '#94a3b8' : '#0f172a' }}>Next →</button>
                  </div>
                </div>
              )}
              </>)}
      ...
      {cartItem && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 24, maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            {added ? (
              ...
              <>
                ...
                {/* Product with image */}
                <div style={{ padding: '10px 14px', background: '#f8fafc', borderRadius: 8, marginBottom: 16, display: 'flex', gap: 12, alignItems: 'center' }}>
                  <ProductThumb src={cartItem.product_image} size={56} />
                  <div>
                    ...
                    {cartItem.price && <div style={{ fontSize: 13, color: '#64748b', marginTop: 3 }}>Listed: <strong style={{ color: '#0f172a' }}>{cartItem.price} {cartItem.currency}</strong>/unit</div>}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    ...
                    <div>
                      <label style={lbl}>Offer Price * <span style={{ color: '#ef4444' }}>per unit</span></label>
                  ...
                  {cartPrice && cartQty && (
                    <div style={{ padding: '8px 12px', background: '#f0fdf4', borderRadius: 6, fontSize: 13 }}>
                      Total: <strong style={{ color: '#15803d' }}>{(parseFloat(cartPrice) * parseInt(cartQty)).toLocaleString()} {cartCur}</strong>
