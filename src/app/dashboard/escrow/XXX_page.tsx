              ...
              {showWdForm && (
                <div style={{ background: 'white', border: '1px solid #fde68a', borderRadius: 10, padding: 20, marginBottom: 16 }}>
                  {wSubmitted ? (
                    ...
                    <>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#0f172a', marginBottom: 6 }}>↩ Withdrawal Request</div>
                      <div style={{ fontSize: 12, color: '#92400e', background: '#fffbeb', padding: '8px 12px', borderRadius: 6, marginBottom: 14 }}>
                        Admin will review your request and determine any applicable withdrawal fee (%) before processing.
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
                        <div>
                          <label style={lbl}>Amount *</label>
                          <input type="number" value={wAmount} onChange={e => setWAmount(e.target.value)} style={inp()} placeholder="0.00" />
                        </div>
                        <div>
                          <label style={lbl}>Currency *</label>
                          <select value={wCurrency} onChange={e => setWCurrency(e.target.value)} style={inp()}>
                            {CURRENCIES.map(c => (
                              <option key={c} disabled={getBalance(c) <= 0}>{c} — {CURRENCY_SYMBOL[c]}{getBalance(c).toFixed(2)} available</option>
                            ))}
                          </select>
                        </div>
                        <div>
                          <label style={lbl}>Bank Name</label>
                          <input value={wBankName} onChange={e => setWBankName(e.target.value)} style={inp()} />
                        </div>
                        <div>
                          <label style={lbl}>IBAN *</label>
                          <input value={wIban} onChange={e => setWIban(e.target.value)} style={inp()} placeholder="GBXX XXXX..." />
                        </div>
                        <div style={{ gridColumn: '1 / -1' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                        <button onClick={submitWithdrawal} disabled={wSubmitting || !wAmount || !wIban}
                          style={{ padding: '10px 20px', background: (!wAmount || !wIban) ? '#94a3b8' : '#92400e', color: 'white', border: 'none', borderRadius: 6, cursor: (!wAmount || !wIban) ? 'not-allowed' : 'pointer', fontSize: 13, fontWeight: 700 }}>
