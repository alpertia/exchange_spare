...
const NEXT_ACTION: Record<string, { buyer: string; seller: string }> = {
  ...
  delivered:     { buyer: 'Waiting for Trade Assurance release',    seller: 'Waiting for Trade Assurance release' },
}
...
export default function TransactionsPage() {
  ...
  async function submitPayment() {
    ...
    if (useEscrow) {
      ...
      if (error || !result?.ok) {
        alert(result?.error || error?.message || 'Trade Assurance payment failed')
    ...
    await logEvent(payTx.id, 'confirmed', 'payment_held', 'buyer', `Payment: ${amt} ${payTx.currency} ${useEscrow ? '(trade-assurance)' : '(direct) ref:' + payRef}`)
  ...
  function ActionButtons({ tx }: { tx: Tx }) {
    ...
    if (s === 'offer_sent') {
      if (isSeller) actions.push(btn('✓ Accept', '#15803d', '#f0fdf4', () => updateTx(tx.id, { status: 'confirmed' })))
    ...
    if (s === 'shipped' && isBuyer) {
      actions.push(btn('✓ Confirm Received', '#15803d', '#f0fdf4', () => updateTx(tx.id, { status: 'delivered' })))
    }

    if (s === 'delivered') {
      actions.push(<div key="wait" style={{ fontSize: 11, color: '#94a3b8', padding: '4px 0' }}>⏳ Awaiting Trade Assurance release</div>)
    }
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            <div style={{ background: 'white', borderRadius: 12, padding: 24, maxWidth: 420, width: '100%' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                <button onClick={() => setUseEscrow(true)} style={{ flex: 1, padding: '8px', background: useEscrow ? '#f0fdf4' : 'white', color: useEscrow ? '#15803d' : '#64748b', border: `1px solid ${useEscrow ? '#15803d' : '#e2e8f0'}`, borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: useEscrow ? 700 : 400 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 24, maxWidth: 420, width: '100%' }}>
            ...
            <div style={{ padding: '8px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 6, fontSize: 12, color: '#dc2626', marginBottom: 14 }}>
              ❄ Funds will be held in Trade Assurance. Platform admin will review and decide within 48h.
            </div>
